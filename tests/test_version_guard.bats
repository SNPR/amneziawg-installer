...
@test "guard: die message updates both files from one fork snapshot" {
    run run_compat "manage_amneziawg.sh" "5.19.2" "5.20.0"
    [[ "$output" == *"git -C /root/amneziawg-installer fetch origin feat/v3"* ]]
    [[ "$output" == *"/root/amneziawg-installer/manage_amneziawg.sh /root/awg/manage_amneziawg.sh"* ]]
    [[ "$output" == *"/root/amneziawg-installer/awg_common.sh /root/awg/awg_common.sh"* ]]
}
