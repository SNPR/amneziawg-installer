...
setup() {
    ...
    # Deliberately use the pre-topology legacy format: no AWG_ROLE and no
    # AWG_UPSTREAM_IFACE. It must continue to load as single + awg1.
...
@test "legacy init without role or upstream iface defaults to single + awg1" {
    require_jq
...
@test "legacy entry init without upstream iface defaults to awg1" {
    require_jq
    printf '%s\n' "export AWG_ROLE='entry'" >> "$TEST_DIR/awg/awgsetup_cfg.init"
    run --separate-stderr bash "$SCRIPT" upstream show --json "${MOCK_ARGS[@]}"
    [ "$status" -eq 0 ]
    printf '%s' "$output" | jq -e '.command == "upstream" and .action == "show" and .interface == "awg1"' >/dev/null
}

@test "duplicate explicit AWG_ROLE fails instead of silently becoming single" {
    require_jq
    printf '%s\n' "export AWG_ROLE='entry'" "AWG_ROLE='single'" >> "$TEST_DIR/awg/awgsetup_cfg.init"
    run --separate-stderr bash "$SCRIPT" list --json "${MOCK_ARGS[@]}"
    [ "$status" -eq 1 ]
    _assert_error_json
}

@test "malformed explicit AWG_ROLE assignment fails closed" {
    require_jq
    printf '%s\n' 'export AWG_ROLE entry' >> "$TEST_DIR/awg/awgsetup_cfg.init"
    run --separate-stderr bash "$SCRIPT" list --json "${MOCK_ARGS[@]}"
    [ "$status" -eq 1 ]
