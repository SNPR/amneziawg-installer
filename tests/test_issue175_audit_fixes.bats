...
@test "issue #175/2: RU/EN post-commit cleanup deletes only an exact owned old-port rule" {
    for f in install_amneziawg.sh install_amneziawg_en.sh; do
        block=$(awk '/^step7_start_service\(\)/,/^}/' "$BATS_TEST_DIRNAME/../$f")
        [[ "$block" == *'delete_owned_ufw_udp_allow_if_present "$PREV_AWG_PORT" "AmneziaWG VPN"'* ]]
...
@test "issue #175/2 functional: firewall block deletes old rule only when port changed" {
    # Extract the deletion guard and run it with the exact-owned helper stubbed
    # to verify both directions: changed port -> helper called; same -> no call.
    local guard
    guard=$(awk '/^    if \[\[ -n "\$\{PREV_AWG_PORT:-\}"/,/^    fi$/' \
        "$BATS_TEST_DIRNAME/../install_amneziawg.sh")
    [ -n "$guard" ]

    run bash -c '
        delete_owned_ufw_udp_allow_if_present() { echo "delete-owned:$1:$2"; return 0; }
        log() { :; }; log_warn() { :; }
        CONFIG_FILE=/nonexistent; _post_commit_cleanup_failed=0
    ...
    '
    [ "$status" -eq 0 ]
    [[ "$output" == *'delete-owned:51820:AmneziaWG VPN'* ]]
    # Exactly one delete call: same-port and empty-prev runs must not delete.
    [ "$(grep -c 'delete-owned:' <<< "$output")" -eq 1 ]
}
...
@test "issue #175/2 reboot: RU/EN post-commit cleanup clears the key only after exact deletion" {
    for f in install_amneziawg.sh install_amneziawg_en.sh; do
        block=$(awk '/^step7_start_service\(\)/,/^}/' "$BATS_TEST_DIRNAME/../$f")
        # The sed cleanup must live inside the exact helper's success branch.
        [[ "$block" == *"sed -i '/^export PREV_AWG_PORT=/d'"* ]]
        success_branch=$(printf '%s\n' "$block" \
            | awk '/if delete_owned_ufw_udp_allow_if_present "\$PREV_AWG_PORT"/,/else/')
        [[ "$success_branch" == *'sed -i'* ]]
    done
}

@test "issue #175/2 reboot functional: failed ufw delete keeps the persisted key for a retry" {
    # Extract the deletion guard and run it against a real config file with the
    # exact helper stubbed: success removes the key, failure leaves it in place.
    local guard
    guard=$(awk '/^    if \[\[ -n "\$\{PREV_AWG_PORT:-\}"/,/^    fi$/' \
        "$BATS_TEST_DIRNAME/../install_amneziawg.sh")
    [ -n "$guard" ]
    ...
    run bash -c '
        ...
        delete_owned_ufw_udp_allow_if_present() { return 1; }
        PREV_AWG_PORT=51820; AWG_PORT=443
        _post_commit_cleanup_failed=0
        '"$guard"'
        ...
        delete_owned_ufw_udp_allow_if_present() { return 0; }
        PREV_AWG_PORT=51820; AWG_PORT=443
        _post_commit_cleanup_failed=0
        '"$guard"'
        if grep -q "^export PREV_AWG_PORT=" "$CONFIG_FILE"; then
