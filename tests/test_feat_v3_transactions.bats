...
@test "feat/v3 RU/EN: policy preflight and runtime reject every earlier RPDB rule and table collision" {
    ...
    for file in "${INSTALLERS[@]}"; do
        ...
        assert_has "$preflight" 'else if (prio ~ /^[0-9]+$/ && prio < pn)' "$file preflight earlier-rule rejection"
        assert_has "$runtime" 'else if (prio ~ /^[0-9]+$/ && prio < pn)' "$file runtime earlier-rule rejection"
