...
> **Upstream merge:** baseline advanced to **bivlked 5.15.6** (DKMS self-repair, dual-stack IPv6, hardened validators, fail2ban fixes, etc.). Conflicts in `awg_common`, `install`, `regenerate_client`, `generate_vpn_uri`, step5 and `validate_port` were resolved keeping every fork feature listed below. The helper-script SHA256 pins were recomputed for the merged files.
...
- **`--warp-bypass=<SPEC>[,<SPEC>...]` — selective WARP exceptions per destination.** Fixes the classic "WARP turned on → YouTube videos stop loading" scenario (Cloudflare IPs get rate-limited by YouTube-CDNs and other large services). SPEC: `none` (default) | `youtube` (YouTube CIDR list from [touhidurrr/iplist-youtube/cidr4.txt](https://raw.githubusercontent.com/touhidurrr/iplist-youtube/main/lists/cidr4.txt) — narrowly scoped to streaming, no collateral Google services) | `custom:<URL|/path>` (arbitrary list via URL or local file). Comma-separated for multiple sources. Each source line's format is auto-detected: CIDR (`1.2.3.4[/N]`) or a domain name (resolved via `@1.1.1.1` and added as `/32`); `#` comments and dnsmasq-style `full:`/`@tag` are accepted. Installer lays down `/usr/local/sbin/awg-warp-bypass.sh` + systemd service + timer (refresh every 6 hours), config in `/etc/amnezia/amneziawg/warp-bypass.conf`, marker `$AWG_DIR/.warp_bypass_enabled_by_installer`, uninstall wipes everything. Only valid with `--egress=warp`; the validator rejects it on `role=entry`.
...
- **`--amnezia-dns=on|off` — AmneziaDNS integration that unlocks site-based split tunneling in the Amnezia VPN client.** Lifts the "Default server does not support split tunneling function" limitation. When `on`, the installer drops `dnsmasq` on the tunnel-gateway IP (first address of `AWG_TUNNEL_SUBNET`, e.g. `10.8.0.1`) as a local resolver for AWG clients, upstream Cloudflare 1.1.1.1 / 1.0.0.1. The client `vpn://` URI is emitted as a "real Amnezia server" (`isThirdPartyConfig:false` + `amnezia-dns` container) — this unlocks the site-list UI in Amnezia VPN, where the user marks individual sites as "bypass VPN" and their DNS is resolved locally on the device. Result: a site from the list sees the user's real IP (mobile / home), not the VPS IP. Marker `$AWG_DIR/.amnezia_dns_enabled_by_installer`; a `systemd-resolved` drop-in with `DNSStubListener=no` is laid down only if there is an actual port-53 collision on `0.0.0.0`. UFW `allow in on awg0 to <gateway> port 53`. Uninstall removes only our own `/etc/dnsmasq.d/amneziawg.conf` and leaves the dnsmasq package alone (the user may have installed it before our installer). Allowed on `--role=single` and `--role=entry`; rejected on `--role=exit` (exit does not hand configs to clients).
