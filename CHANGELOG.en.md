...
> **SNPR fork:** the baseline is synchronized with `bivlked` upstream through **bb6fb79** (v5.28.1 release code plus subsequent upstream changes). The fork-specific history is preserved below; the upstream release entries follow unchanged.
...
- **`--warp-bypass=<SPEC>[,<SPEC>...]` — selective WARP exceptions per destination.** Fixes the classic "WARP turned on → YouTube videos stop loading" scenario. SPEC: `none` (default) | `youtube` | `custom:<URL|/path>`. Source lines may be CIDRs or domain names; external input is capped at 2 MiB per source, 2048 routes, one million addresses, and public prefixes no wider than `/19`. The installer deploys a service and timer, records an exact owned-route ledger, and refuses to delete or overwrite anything when that ledger is malformed. Only valid with `--egress=warp`; the validator rejects it on `role=entry`.
- **Universal TCPMSS clamp on FORWARD in WARP mode** (no `-o wgcf` qualifier): previously the bypass path `awg0 → eth0` let through large TCP segments with `MSS=1460` from YouTube/Google, and the return flow via awg0 (MTU 1280) dropped them — PMTU discovery was blackholed by upstream ICMP filters and videos wouldn't load. The clamp is now applied without `-o` matching and keeps MSS within the MTU of whichever path the packet actually takes.
- **`--amnezia-dns=on|off` — AmneziaDNS integration that unlocks site-based split tunneling in the Amnezia VPN client.** When enabled, the installer binds `dnsmasq` to the tunnel gateway and creates its own `systemd-resolved` drop-in only when needed. Setup is transactional: failures restore the previous files and service states. A versioned marker separately records ownership of each UFW rule, and uninstall removes only proven-owned resources while leaving the dnsmasq package installed. Allowed on `--role=single` and `--role=entry`; rejected on `--role=exit`.
- **Cloudflare WARP egress for exit- and single-nodes.** New flag `--egress=direct|warp` (default `direct`). With `--egress=warp` the installer:
  1. Downloads pinned [wgcf](https://github.com/ViRb3/wgcf) v2.2.32 (amd64 / arm64 / armv7) and verifies an architecture-specific SHA-256 before installation.
  ...
  4. Adds fail-closed policy routing to `awg0.conf`: a persistent blackhole default, a lower-metric working default through wgcf, a source rule with a separate blackhole guard, MASQUERADE, and calculated `TCPMSS --set-mss`. If WARP goes down, client traffic cannot leak through the main route; the only exception is an explicitly listed `--warp-bypass` destination, which is direct by definition.
- **Companion flags:** `--warp-table=N` (default `2408`) and `--warp-priority=N` (default `789`, valid range `1..32764`: the next priority is reserved for the guard and `32766` is the system `main` lookup). WARP and entry/upstream modes are mutually exclusive, so their tables are never active together.
...
- **UFW:** WARP egress adds `ufw route allow in on awg0 out on wgcf`; the direct `awg0 → <nic>` route is allowed only when a non-empty `--warp-bypass` is configured.
- **Uninstall** uses separate ownership markers for the wgcf service, configuration, account file, and binary. An empty legacy service marker is ambiguous and no longer authorizes deletion of an existing unit.
- **Tests:** `tests/test_warp_egress.bats` covers route rendering, the fail-closed fallback, direct/entry precedence, safe loading, custom table/priority, WARP ownership, and the pinned wgcf download.
...
- **Cascade tests:** `tests/test_upstream.bats` covers interface names, safe `[Interface]`/`[Peer]` parsing, Table/FwMark, fail-closed policy routing, incomplete AWG 2.0 sets, metacharacters, and entry/single rendering.
- **Contract suite `tests/test_feat_v3_transactions.bats`:** pins the install transaction boundaries, restart-safe step 7 resume, strict policy-routing/firewall verification, and ownership-aware teardown for chains and WARP.

### Fixed

- **WARP/cascade live verification accepts numeric blackhole actions:** `ip -N -4 rule show` prints `FR_ACT_BLACKHOLE` as `6`, while the validator only accepted `blackhole`, rejecting a correctly installed guard after tunnel startup. RU/EN now accept both representations with unchanged source, priority, and field-count checks; tests cover numeric output and reject other actions. Live-check failures identify the specific service/link/rule/route/firewall condition, and the UP check requires nonempty `ip link show up` output.
...
- **Failures before the first WARP mutation no longer restart a live tunnel, and a partial write cannot be published:** dirty state is distinct from a read-only snapshot; write/ENOSPC failures preserve the previous upstream config and WARP-bypass ledger, followed by a verified rollback.

### Changed

- **The fork is synchronized with upstream v5.28.1:** every upstream change since the common base is included, including AWG 3.x, I1-I5, boot-package-safe upgrades, strict JSON/CLI contracts, and restore/IPv6 fixes, while chains, WARP, AmneziaDNS, and custom ports remain intact.
- **Network-mode changes are transactional:** the new `awg0.conf` is rendered and validated in staging first; failures after the interface is stopped restore the previous config, dependency drop-in, and support unit. A systemd dependency enforces WARP/upstream → awg0 ordering at boot and on restart.
- **`manage restart` and `upstream apply/restart` complete only after a live health check:** the device-parameter snapshot and success message are written after the active unit/link is confirmed, not immediately after issuing the restart command.
