...
При запуске от root значения `--conf-dir` и `--server-conf` принимаются только
для обычных файлов/каталогов, принадлежащих root, без symlink и без права записи
для группы/остальных; вся родительская цепочка пути должна удовлетворять тем же
условиям. Проверка выполняется до записи журнала и до загрузки `awg_common.sh`.
...
* **`upstream <show|up|down|restart|apply>`:** Управлять upstream-туннелем на `role=entry`. `down` сначала гарантированно снимает `awg0`, включая вручную поднятый live-интерфейс, и только затем upstream. После `up`/`restart`/`apply` прежнее активное состояние `awg0` восстанавливается; при отказе upstream основной интерфейс принудительно гасится (fail-closed).
...
С v5.21.0 флаг `--json` поддерживают не только `list`/`stats`, но и команды управления: `add`, `remove`, `regen`, `modify`, `backup`, `restore`, `check` (алиас `status`), `restart`, `upstream`, `repair-module` (алиас `repair`). Правила контракта:
...
- `upstream` возвращает `action`, `interface`, `upstream_active` и `awg0_active`. Поля `*_active` истинны, если активен systemd-unit **или** соответствующий live-интерфейс существует; поэтому вручную поднятый туннель не маскируется как выключенный. Источником истины об успехе остаются `ok` и exit-код.
...
#### Привязка вспомогательных файлов

В форке installer берёт `awg_common.sh` и `manage_amneziawg.sh` из своей ветки:

```
https://raw.githubusercontent.com/SNPR/amneziawg-installer/feat/v3/awg_common.sh
```

Подвижный ref сам по себе не является supply-chain pinning, поэтому штатный installer сверяет оба файла с вшитыми SHA-256 и отказывается устанавливать несовпавшие байты. При запуске из клона он использует локальную пару из одного Git-снимка и тоже проверяет хеши.
...
Любое переопределение `AWG_REPOSITORY` или `AWG_BRANCH` сознательно отключает
проверку по встроенным SHA-пинам: загруженные helper-файлы остаются
неверифицированными. Используйте override только с доверенной development-веткой;
для штатной установки оставляйте закреплённые `SNPR/amneziawg-installer@feat/v3`.
...
Для обновления скриптов управления и общей библиотеки **без переустановки сервера** берите оба файла из одного Git-снимка:
...
# Русская версия (для английской добавьте _en у двух исходников):
...
<details>
  ...
  <b>О:</b> Это собирается каскадом из двух серверов: клиент подключается к серверу-входу (лучше в РФ), российский трафик уходит напрямую с него, остальное - через второй сервер за границей. Именно <em>раздельный по назначениям</em> каскад не входит в установщик и настраивается по <a href="CASCADE.md">CASCADE.md</a>. Не путайте его со встроенным режимом <code>--role=entry</code>: тот отправляет через вторую VPS весь IPv4-трафик клиентов и описан в <a href="MULTIHOP.md">MULTIHOP.md</a>.
</details>
