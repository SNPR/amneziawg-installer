...
<p align="center">
  <a href="https://bivlked.github.io/amneziawg-installer/"><img src="https://img.shields.io/badge/Upstream_Website-bivlked.github.io-3ddc97" alt="Upstream project website"></a>
...
This branch is synchronized with `bivlked` upstream through commit **bb6fb79** (the v5.28.1 release code plus subsequent upstream documentation/release-process changes), including boot-critical package guards, the safer system upgrade, diagnostic secret filtering, and AmneziaWG 3.x support. It adds the opt-in modes below; without their flags, the upstream-compatible `single` + `direct` mode remains in effect.

<a id="diff"></a>
### Fork modes and notable flags
...
| **Privileged ports** | `--port=443` (also 53, 500) | DPI-friendly ports for mobile carriers; the full 1–65535 range is available |
...
Clone the branch into a directory other than `/root/awg` (that path is the installed server working directory):
...
Alternatively, download the fork's standalone installer. At step 5 it fetches
`common/manage` from the same branch and verifies their pinned SHA256 hashes:

```bash
wget -O install_amneziawg_en.sh https://raw.githubusercontent.com/SNPR/amneziawg-installer/feat/v3/install_amneziawg_en.sh
sudo bash ./install_amneziawg_en.sh
```

When run from a clone, step 5 prefers the local `awg_common_en.sh` and `manage_amneziawg_en.sh`, so the fork extensions are the files installed on the server. The pinned `wget` command in the next section installs plain upstream and does not include these extensions.

Example opt-in modes for a single server:

```bash
sudo bash ./install_amneziawg_en.sh --egress=warp --warp-bypass=youtube --amnezia-dns=on --i1-mode=quic --port=443
```

> ⚠️ `--amnezia-dns=on` unlocks the client-side “bypass VPN” site list: those sites go direct and see the device’s real IP. For a two-VPS chain, routing details, and diagnostics, use [MULTIHOP.en.md](MULTIHOP.en.md).
...
| `upstream` | `<show\|up\|down\|restart\|apply>` | Manage the cascade on `role=entry` (fail-closed) | By action |

> **💡 Note:** `add` and `remove` commands auto-apply changes via `awg syncconf` - no service restart needed.

> **🤖 For scripts and bots:** since v5.21.0 nearly every management command (including `upstream`) understands `--json` and replies with a single JSON object, even when something goes wrong - so your script or bot can parse the output without guesswork. For `upstream`, `upstream_active`/`awg0_active` account for both the systemd unit and a manually raised live link. `list --json` and `stats --json` still return plain arrays, as before. The format and the strict confirmation mode `AWG_STRICT_CONFIRM=1` are covered in [ADVANCED.en.md → JSON interface](ADVANCED.en.md#json-api-adv), and there is already a Telegram bot built on this interface - [awgram](#ecosystem).
...
<details>
  ...
  <b>A:</b> Update <code>manage_amneziawg.sh</code> and <code>awg_common.sh</code> as a pair from the same Git commit. If you already cloned the repository as shown above:
  <pre>
  cd /root/amneziawg-installer
  ...
  # English version (drop _en from both sources for Russian):
  install -m 700 manage_amneziawg_en.sh /root/awg/manage_amneziawg.sh.new
  install -m 700 awg_common_en.sh /root/awg/awg_common.sh.new
  mv -f /root/awg/manage_amneziawg.sh.new /root/awg/manage_amneziawg.sh
  mv -f /root/awg/awg_common.sh.new /root/awg/awg_common.sh
  </pre>
  This keeps both files on one verified Git snapshot; downloading them with two independent <code>wget</code> calls from a moving branch is unsafe. Server reinstallation is not required.
  <br><br>
