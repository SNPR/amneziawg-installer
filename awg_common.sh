...
render_client_config() {
    ...
    # IPv6 leak prevention. Если хост отключил v6 (DISABLE_IPV6=1 — наш дефолт)
    # но клиент подключается с мобильного оператора / роутера, выдающего v6,
    # happy-eyeballs сперва попробует v6 и уйдёт в интернет МИМО туннеля —
    # внешний сайт увидит оригинальный v6-адрес клиента, а не наш сервер/WARP.
    # Добавляем ::/0 в AllowedIPs: клиентское ядро завернёт ВЕСЬ v6-трафик
    # в туннель; т.к. туннель v4-only, v6-пакеты просто дропаются на клиенте,
    # до оператора не доходят, утечки нет. При DISABLE_IPV6=0 (v6 разрешён)
    # не трогаем — тогда v6 пользователь настраивает сам.
    # Идемпотентно: не добавляем ::/0 если в списке уже есть v6-роут
    # (двоеточие в строке — уникальный маркер IPv6 CIDR).
