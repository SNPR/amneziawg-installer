...
- **IPv6 leak у клиентов при `DISABLE_IPV6=1`.** Если хост установлен с отключённым IPv6 (наш дефолт), а клиент подключается с мобильного оператора или роутера, раздающего v6, happy-eyeballs выбирал v6-путь **мимо туннеля** — внешний сайт видел настоящий мобильный v6-адрес клиента, а не IP VPS/WARP. `render_client_config` теперь дописывает `::/0` в `AllowedIPs` клиентского конфига, если `DISABLE_IPV6=1` и v6-роута там ещё нет. Клиентское ядро перехватывает весь v6-трафик в v4-only туннель, где он тихо дропается — до оператора ничего не уходит, утечки нет. Идемпотентно: `::/0` не дублируется если в AllowedIPs уже есть любой v6-CIDR. Существующим установкам достаточно `manage_amneziawg.sh regen`, чтобы правило применилось ко всем клиентам.
