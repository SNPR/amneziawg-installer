...
> **Форк SNPR:** базовая версия синхронизирована с upstream `bivlked` до **bb6fb79** (релизный код v5.28.1 плюс последующие upstream-правки). Ниже сохранена история собственных возможностей форка; записи релизов upstream идут следом без изменений.
...
- **`--warp-bypass=<SPEC>[,<SPEC>...]` — селективный обход WARP для конкретных dst.** Решает типовую проблему «WARP включили — YouTube перестал грузить видео» (Cloudflare IP рейт-лимитят YouTube-CDN'ы и другие крупные сервисы). SPEC: `none` (умолч.) | `youtube` (CIDR-список YouTube из [touhidurrr/iplist-youtube/cidr4.txt](https://raw.githubusercontent.com/touhidurrr/iplist-youtube/main/lists/cidr4.txt) — узкая выборка именно под стриминг, без побочных Google-сервисов) | `custom:<URL|/path>` (произвольный список по URL или локальному файлу). В содержимом источника автоопределяется CIDR (`1.2.3.4[/N]`) или доменное имя (резолвится через `@1.1.1.1` и добавляется как `/32`); поддерживаются комментарии `#` и dnsmasq-стиль `full:`/`@tag`. Внешний ввод ограничен 2 MiB на источник, 2048 маршрутами, миллионом адресов и публичными префиксами не шире `/19`. Установщик ставит `/usr/local/sbin/awg-warp-bypass.sh` + systemd service + timer, ведёт точный ledger собственных маршрутов и при повреждённом ledger ничего не удаляет. Работает только с `--egress=warp`; на `role=entry` валидатор его не пропустит.
...
- **`--amnezia-dns=on|off` — интеграция AmneziaDNS для site-based split tunneling в клиенте Amnezia VPN.** Снимает ограничение Amnezia VPN «Default server does not support split tunneling function». При `on` установщик ставит `dnsmasq` на tunnel-gateway IP и при необходимости собственный `systemd-resolved` drop-in. Настройка транзакционна: при ошибке возвращаются прежние конфиги и состояния сервисов. Versioned marker отдельно фиксирует владение UFW-правилами; uninstall удаляет только доказанно созданные ресурсы и не удаляет пакет dnsmasq. Допустимо на `--role=single` и `--role=entry`; на `--role=exit` отвергается.
- **WARP egress (Cloudflare) для exit- и single-ноды.** Новый флаг `--egress=direct|warp` (по умолчанию `direct`). При `--egress=warp` установщик:
  1. Скачивает закреплённый [wgcf](https://github.com/ViRb3/wgcf) v2.2.32 (amd64 / arm64 / armv7) и до установки проверяет отдельный SHA-256 для архитектуры.
  ...
  4. Включает `wg-quick@wgcf` и дописывает в PostUp/PostDown `awg0.conf` policy routing: постоянный blackhole-default как fail-closed fallback, рабочий default через wgcf с меньшей metric, source-rule с отдельным blackhole-guard, MASQUERADE и вычисленный `TCPMSS --set-mss`. При падении WARP клиентский трафик не утечёт через main route; исключение — только явно перечисленные `--warp-bypass`-назначения, которые по определению идут напрямую.
- **Сопутствующие флаги:** `--warp-table=N` (по умолчанию `2408`) и `--warp-priority=N` (по умолчанию `789`, допустимо `1..32764`: следующий приоритет занят защитным правилом, а `32766` — системным lookup `main`). WARP и entry/upstream взаимоисключающие, поэтому их таблицы одновременно не активны.
...
- **UFW:** при egress=warp добавляется `ufw route allow in on awg0 out on wgcf`; прямой маршрут `awg0 → <nic>` разрешается только когда настроен непустой `--warp-bypass`.
- **Uninstall** использует отдельные marker'ы владения сервисом, конфигом, account-файлом и бинарём wgcf. Пустой legacy marker сервиса считается неоднозначным и не даёт права удалять существующий unit.
- **Тесты:** `tests/test_warp_egress.bats` покрывает рендер маршрутов, fail-closed fallback, direct/entry precedence, safe-load, custom table/priority, WARP ownership и закреплённую загрузку wgcf.
...
- **Тесты для каскада:** `tests/test_upstream.bats` покрывает имена интерфейсов, безопасный парсинг `[Interface]`/`[Peer]`, Table/FwMark, fail-closed policy routing, неполные AWG 2.0 наборы, metacharacters и рендер entry/single.
- **Контрактный набор `tests/test_feat_v3_transactions.bats`:** фиксирует транзакционные границы установки, restart-safe resume шага 7, строгую проверку policy routing/firewall и ownership-aware teardown для цепочек и WARP.

### Исправлено

- **Live-проверка WARP/каскада принимает числовой `blackhole`:** `ip -N -4 rule show` печатает действие `FR_ACT_BLACKHOLE` как `6`, а валидатор ожидал только слово `blackhole`. Поэтому штатный guard ошибочно отклонялся после запуска туннелей. RU/EN теперь принимают оба представления с прежней строгой проверкой источника, приоритета и числа полей; тесты покрывают числовой вывод и отклонение других действий. При отказе live-проверки лог указывает конкретный service/link/rule/route/firewall, а UP-проверка требует непустого ответа `ip link show up`.
- **Исправлен ложный конфликт policy-routing на шаге 7 для WARP и каскада:** после удаления `:` AWK сравнивал приоритеты как строки и считал системные правила `32766`/`32767` более ранними, чем `789`/`456`. Обе проверки (до запуска и после него) в RU/EN теперь явно сравнивают числа; защита от реальных конфликтов сохранена. Регрессионный тест запускает сами AWK-валидаторы на синтетических правилах, не исполняя installer.
- **Повтор установки после аварии на шаге 7 больше не перезапускает уже подтверждённый live `awg0`:** установщик сверяет unit, link, regular config, `awg-quick strip`, service postcondition и exact egress state; неоднозначное состояние оставляется нетронутым. Для переиспользуемого upstream rollback заново взводится и после перезапуска процесса.
- **Переходы между direct, entry/cascade и WARP закрыты от частичного состояния:** конфиги, runtime/link/enabled state, dependency drop-in, UFW, DNS и WARP-bypass входят в согласованный rollback; незавершённые cleanup marker'ы блокируют вторую миграцию до безопасного завершения первой.
- **Policy routing и firewall проверяются по точной форме:** чужие rules/routes, занятые приоритеты, неоднозначные default routes, `linkdown`/неактивные nexthop-интерфейсы и неполные FORWARD/NAT/TCPMSS/isolation rules не считаются успехом. Деинсталляция и post-commit cleanup останавливают и удаляют только ресурсы с доказанным владением.
- **Отказы до первой WARP-мутации больше не перезапускают действующий tunnel, а частичная запись не может быть опубликована:** dirty-state отделён от read-only snapshot; ошибки записи/ENOSPC сохраняют прежний upstream-конфиг и ledger WARP-bypass, после чего выполняется проверяемый rollback.

### Изменено

- **Форк синхронизирован с upstream v5.28.1:** перенесены все изменения upstream от общей базы, включая AWG 3.x, I1-I5, безопасное обновление загрузочных пакетов, строгие JSON/CLI-контракты и исправления restore/IPv6, при этом сохранены цепочки, WARP, AmneziaDNS и custom ports.
- **Переключение сетевых режимов стало транзакционным:** новый `awg0.conf` сначала рендерится и проверяется в staging; при сбое после остановки интерфейса возвращаются прежний конфиг, dependency drop-in и support-unit. systemd dependency гарантирует порядок WARP/upstream → awg0 при загрузке и restart.
- **`manage restart` и `upstream apply/restart` завершаются только после live health-check:** снимок параметров устройства и сообщение об успехе записываются после подтверждения активного unit/link, а не сразу после команды перезапуска.
