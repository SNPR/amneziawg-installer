...
> **Слияние с апстримом:** базовая версия поднята до **bivlked 5.15.6** (DKMS-самовосстановление, dual-stack IPv6, усиленные валидаторы, fail2ban-фиксы и пр.). Конфликты в `awg_common`, `install`, `regenerate_client`, `generate_vpn_uri`, step5 и `validate_port` разрешены с сохранением всех доработок форка ниже. SHA256-пины helper-скриптов пересчитаны под объединённые файлы.
