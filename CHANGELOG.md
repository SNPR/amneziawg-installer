...
- **`--amnezia-dns=on|off` — интеграция AmneziaDNS для site-based split tunneling в клиенте Amnezia VPN.** Снимает ограничение Amnezia VPN «Default server does not support split tunneling function». При `on` установщик ставит `dnsmasq` на tunnel-gateway IP (первый адрес `AWG_TUNNEL_SUBNET`, напр. `10.8.0.1`) — локальный резолвер для AWG-клиентов, upstream Cloudflare 1.1.1.1 / 1.0.0.1. Клиентский `vpn://` URI генерится как «родной Amnezia-сервер» (`isThirdPartyConfig:false` + контейнер `amnezia-dns`) — в UI Amnezia VPN открывается сайт-список, где пользователь помечает сайты как «в обход VPN», и их DNS резолвится локально на устройстве. Итог: сайт из списка видит реальный IP пользователя (мобильный/домашний), а не IP VPS. Маркер `$AWG_DIR/.amnezia_dns_enabled_by_installer`, `systemd-resolved` drop-in с `DNSStubListener=no` кладётся только при реальной коллизии на `0.0.0.0:53`. UFW `allow in on awg0 to <gateway> port 53`. Uninstall сносит только свой конфиг `/etc/dnsmasq.d/amneziawg.conf`, пакет dnsmasq не удаляет (мог быть у пользователя ДО нашего инсталлятора). Допустимо на `--role=single` и `--role=entry`; на `--role=exit` отвергается (exit не раздаёт конфиги клиентам).
