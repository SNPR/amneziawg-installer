...
> **Форк SNPR/amneziawg-installer** добавляет multi-hop каскад, Cloudflare WARP egress на exit-ноде и агрессивный QUIC-маскарад для мобильного DPI. Рецепт ниже поднимает полную связку: клиенты подключаются к **Ноде 2** на UDP 500, трафик идёт через **Ноду 1**, а наружу выходит через **Cloudflare WARP** — внешние сайты видят IP Cloudflare, не VPS.
>
> Понадобится: **2 VPS** (Ubuntu 24.04/25.10 или Debian 12/13), root-доступ по SSH. Далее по тексту Нода 1 (exit) обозначается как `IP1`, Нода 2 (entry) — как `IP2`.
...
Можно опустить если WARP для тебя сам по себе достаточен. Автообновление списка раз в 6 часов через systemd timer.
