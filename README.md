...
<p align="center">
  <a href="https://bivlked.github.io/amneziawg-installer/ru/"><img src="https://img.shields.io/badge/Upstream_Website-bivlked.github.io-3ddc97" alt="Upstream project website"></a>
  ...
  <a href="https://github.com/SNPR/amneziawg-installer/blob/feat/v3/LICENSE"><img src="https://img.shields.io/badge/license-MIT-green" alt="License"></a>
  <a href="https://github.com/bivlked/amneziawg-installer/releases"><img src="https://img.shields.io/badge/Upstream_Installer_Version-5.28.1-blue" alt="Upstream installer version"></a>
  <a href="https://github.com/SNPR/amneziawg-installer/actions/workflows/test.yml?query=branch%3Afeat%2Fv3"><img src="https://github.com/SNPR/amneziawg-installer/actions/workflows/test.yml/badge.svg?branch=feat%2Fv3" alt="SNPR feat/v3 tests"></a>
  <a href="https://github.com/bivlked/amneziawg-installer/stargazers"><img src="https://img.shields.io/github/stars/bivlked/amneziawg-installer?style=flat&amp;label=Upstream%20stars" alt="Upstream stars"></a>
  <img src="https://img.shields.io/github/last-commit/bivlked/amneziawg-installer?label=Upstream%20last%20commit" alt="Upstream last commit">
  <a href="https://deepwiki.com/bivlked/amneziawg-installer"><img src="https://img.shields.io/badge/Upstream-Ask_DeepWiki-1f6feb" alt="Upstream DeepWiki"></a>
</p>
...
## 🧩 Возможности форка SNPR

Эта ветка синхронизирована с upstream `bivlked` до коммита **bb6fb79** (релизный код v5.28.1 и последующие upstream-правки документации/релизного процесса) — включая защиту загрузочных пакетов, безопасное обновление системы, фильтрацию секретов в диагностике и поддержку AmneziaWG 3.x — и добавляет опциональные режимы ниже. Без специальных флагов остаётся обычный upstream-совместимый режим `single` + `direct`.

<a id="diff"></a>
### Режимы форка и важные флаги
...
| **Привилегированные порты** | `--port=443` (и 53, 500) | Обход DPI мобильных операторов; доступен весь диапазон 1–65535 |
...
### Установка именно форка

Клонируйте ветку в каталог, отличный от `/root/awg` (это рабочий каталог установленного сервера):
...
Либо скачайте одиночный installer форка — на шаге 5 он заберёт `common/manage`
из той же ветки и проверит их закреплённые SHA256:

```bash
wget -O install_amneziawg.sh https://raw.githubusercontent.com/SNPR/amneziawg-installer/feat/v3/install_amneziawg.sh
sudo bash ./install_amneziawg.sh
```

При запуске из клона шаг 5 предпочитает локальные `awg_common.sh` и `manage_amneziawg.sh`, поэтому на сервер попадают именно расширения форка. Закреплённая `wget`-команда в следующем разделе по-прежнему устанавливает чистый upstream и этих расширений не содержит.

Пример опциональных режимов для одного сервера:

```bash
sudo bash ./install_amneziawg.sh --egress=warp --warp-bypass=youtube --amnezia-dns=on --i1-mode=quic --port=443
```

> ⚠️ `--amnezia-dns=on` разблокирует клиентский список сайтов «в обход VPN»: такие сайты идут напрямую и видят реальный IP устройства. Для цепочки из двух VPS, схемы маршрутизации и диагностики используйте [MULTIHOP.md](MULTIHOP.md).
...
| `upstream` | `<show\|up\|down\|restart\|apply>` | Управление каскадом на `role=entry` (fail-closed) | По действию |

> **💡 Примечание:** Команды `add` и `remove` автоматически применяют изменения через `awg syncconf` - перезапуск сервиса не требуется.

> **🤖 Для скриптов и ботов:** с v5.21.0 почти все команды управления (включая `upstream`) понимают `--json` и отвечают одним JSON-объектом, даже если что-то пошло не так - вывод можно спокойно разбирать из своего скрипта или бота. У `upstream` поля `upstream_active`/`awg0_active` учитывают и systemd-unit, и вручную поднятый live-интерфейс. `list --json` и `stats --json`, как и раньше, отдают простые массивы. Формат и строгий режим подтверждений `AWG_STRICT_CONFIRM=1` расписаны в [ADVANCED.md → JSON-интерфейс](ADVANCED.md#json-api-adv), а готовый Telegram-бот на этом интерфейсе уже есть - [awgram](#ekosistema).
...
<details>
  ...
  <b>О:</b> Обновляйте <code>manage_amneziawg.sh</code> и <code>awg_common.sh</code> только парой из одного Git-коммита. Если репозиторий уже клонирован по инструкции выше:
  <pre>
  cd /root/amneziawg-installer
  git fetch origin feat/v3
  git checkout feat/v3
  git pull --ff-only origin feat/v3

  # Русская версия (для английской добавьте суффикс _en у двух исходников):
  install -m 700 manage_amneziawg.sh /root/awg/manage_amneziawg.sh.new
  install -m 700 awg_common.sh /root/awg/awg_common.sh.new
  mv -f /root/awg/manage_amneziawg.sh.new /root/awg/manage_amneziawg.sh
  mv -f /root/awg/awg_common.sh.new /root/awg/awg_common.sh
  </pre>
  Так оба файла берутся из одного проверенного Git-снимка; скачивать их двумя независимыми <code>wget</code> с изменяемой ветки небезопасно. Переустановка сервера не требуется.
  <br><br>
