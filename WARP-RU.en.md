...
- IPv6 on the client device is a separate layer, and this scheme does not touch it. These instructions go on top of an existing install and do not change the client's mode, and in split routing (the default) `::/0` never reaches the client config - so the device's IPv6 goes around the tunnel, and a Russian site sees your real home address directly, with no scripts involved. To check from the device: `curl -6 ifconfig.co` - if your home address comes back, IPv6 is going around. Details in [ADVANCED.en.md](ADVANCED.en.md#split-detect-adv).
